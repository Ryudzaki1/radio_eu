...
Админка доступна по `/simsim`. После логина сервер выдает HTTP-only cookie
`admin_session`, а все изменяющие запросы из админки проходят с CSRF-токеном из
`/api/admin/session`. Если токен отсутствует или не совпадает, сервер отвечает
`403`.

Вкладка `Доходы` показывает:

- оплаты вопросов через Telegram Stars;
- платные реакции Stars в канале `@baa_vipe`;
- транзакции Stars, которые прислал бот;
- события воронки за последние семь дней.

Для проверки базовой цепочки оплаты в админке есть вкладка `Тесты` и кнопка
`Тест оплаты вопроса`. Этот тест не списывает реальные Stars.
...
music/chill/live - новая библиотека BAA Vibe Chill для будущей live-ротации
music/chill/play - новая библиотека BAA Vibe Chill для музыки по заказу
music/chill/jingles - короткие отбивки BAA Vibe
music/chill/transitions - переходы между блоками эфира
```

По умолчанию live-ротация теперь берет музыку из `music/chill/live`
(`MUSIC_ROTATION_SOURCE=catalog`). Если новая папка пустая, сервер автоматически
возвращается к старой папке `music/live`, чтобы эфир не остался без музыки.
Fallback можно выключить переменной `MUSIC_CATALOG_FALLBACK_TO_LEGACY=false`.

`music/live` остается совместимой старой live-папкой.

`music/play` используется для ручных вставок. Сервер проверяет, что файл реально
существует, перед постановкой в очередь.

В админке вкладка `Аудио Файлы` показывает две зоны:

- `BAA Vibe Chill Radio` - новая нейромузыкальная библиотека по ролям:
  `Live эфир`, `Play вставки`, `Jingles`, `Transitions`;
- `Текущий эфир сейчас` и `Текущие Play вставки` - папки, которые уже
  подключены к текущей ротации и ручным вставкам;
- `Аудио ведущего` пока отображает архив озвучек только для прослушивания.

Пока генерация эфирной музыки идет вручную в личном кабинете ElevenLabs,
готовые файлы можно загружать через админку. API-генерация музыки будет
подключаться отдельно для заказной музыки слушателей.
